...
def upgrade():
    ...
    # If core contact/lead/appointment tables already exist (e.g. on a running prod DB),
    # skip this migration to avoid duplicate type / table creation.
    from sqlalchemy import inspect
    inspector = inspect(bind)
    existing_tables = set(inspector.get_table_names())
    if {"contact_cards", "leads", "appointments"}.issubset(existing_tables):
        return
    ...
    if dialect != 'sqlite':
        # Be defensive: even if enum_exists returns False due to search_path issues,
        # swallow DuplicateObject errors so migrations remain idempotent.
        try:
    ...
    # Note: Use simple String columns here to avoid hard dependency on PostgreSQL ENUM types.
    # The SQLAlchemy models use native_enum=False, so VARCHAR is sufficient.
    op.create_table(
        "leads",
        sa.Column("id", sa.String(), primary_key=True),

...
async def get_unassigned_calls(company_id: str, db: Session = Depends(get_db)):
...
async def add_call(request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):

#!/usr/bin/env python3
"""
Test CallRail v3 API setup with corrected configuration
"""
import os
import sys
import requests
...
def test_callrail_api_connection():
    """Test CallRail v3 API connection with different account ID formats"""
    ...
    try:
        ...
        account_formats = [
            account_id,  # Original: 307-566-904
            account_id.replace("-", ""),  # 307566904
            account_id.replace("-", "_"),  # 307_566_904
        ]
        
        for acc_id in account_formats:
            print(f"🔍 Trying account ID format: {acc_id}")
            
            # Test API connection with account info
            url = f"https://api.callrail.com/v3/accounts/{acc_id}.json"
            headers = {
        ...
        print("❌ All account ID formats failed")
        return False
...
def test_callrail_tracking_numbers():
    """Test CallRail tracking numbers with different account ID formats"""
    ...
    try:
        ...
        for acc_id in account_formats:
            ...
            url = f"https://api.callrail.com/v3/accounts/{acc_id}/tracking_numbers.json"
            headers = {
                "Authorization": f"Token token={api_key}",
                "Content-Type": "application/json"
            }
        ...
        print("❌ All account ID formats failed for tracking numbers")
        return False
...
def test_callrail_integration():
    ...
    try:
        ...
        required_routes = [
            "/callrail/call.incoming",
            "/callrail/call.answered",
            "/callrail/call.missed", 
...
def main():
    ...
    print("🚀 CallRail v3 API Setup Test Suite (Corrected)")
    print("=" * 60)
    ...
    if passed >= 2:  # At least webhook and integration should work
